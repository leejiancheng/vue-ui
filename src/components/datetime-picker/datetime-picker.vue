<!-- **
 * mt-datetime-picker
 * @module components/datetime-picker
 * @desc 日期时间选择器
 * ********** API **********
 * @param {String} [type=datetime] - 组件的类型(可选值: datetime, date, time)
 * @param {String} [cancelText=取消] - 取消按钮文本
 * @param {String} [confirmText=确定] - 确定按钮文本
 * @param {Date} [startDate=十年前的1月1日] - 日期的最小可选值
 * @param {Date} [endDate=十年后的12月31日] - 日期的最大可选值
 * @param {Number} [startHour=0] - 小时的最小可选值
 * @param {Number} [endHour=23] - 小时的最大可选值
 * @param {String} [yearFormat={value}] - 年份模板
 * @param {String} [monthFormat={value}] - 月份模板
 * @param {String} [dateFormat={value}] - 日期模板
 * @param {String} [hourFormat={value}] - 小时模板
 * @param {String} [minuteFormat={value}] - 分钟模板
 * ********** Events **********
...
<template>
	<mt-popup v-model="visible" position="bottom" class="mint-datetime">
		<mt-picker class="mint-datetime-picker" ref="picker" :slots="dateSlots" @change="onChange" :visible-item-count="visibleItemCount" show-toolbar>
			<span class="mint-datetime-action mint-datetime-cancel" @click="visible = false;$emit('cancel')">
				{{cancelText}}
			</span>
...
<script>
	...
	export default {
		...
		components: {"mt-picker": Picker, "mt-popup": Popup},
		props: {
			cancelText: {
				type: String,
				default: "取消"
		...
		methods: {
			...
			handleExceededValue() {
				...
				this.dateSlots.filter(child => child.vaules !== undefined).map(slot => slot.values).forEach((slotValues, index) => {
